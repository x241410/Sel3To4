...
public class WLSC_169212_WNP_REG_AnHYCancelFromPAC extends BaseTest {
	...
	public void testMethod_AnHYCancelFromPAC(ITestContext iTestContext) throws Exception {
		 ...
		 * PACSearchPageSteps.verifyNoPortReqForAnotherSubFromPAC(pacTestData,
		 * foreignSub); WebDriverSteps.closeTheBrowser();
		 * Reporting.printAndClearLogGroupStatements();
		 * 
		 ...
		 * RCMPageSteps.verifyAnotherSubscriberProvisioningStatus(rcmTestData,
		 * foreignSub, "inactive"); WebDriverSteps.closeTheBrowser();
		 * Reporting.printAndClearLogGroupStatements();
		 * 
		 ...
		 * "active"); CODASearchPageSteps.verifyAnotherSubStatusFromCODA(codaTestData,
		 * foreignSub, "inactive"); WebDriverSteps.closeTheBrowser();
		 * Reporting.printAndClearLogGroupStatements();
		 * 
		 ...
		 * Reporting.logReporter(Status.INFO,
		 * "STEP === SPID Validation --> SPID validation check via API call===");
		 * 
		 ...
		 * apiOperation.get("spidAPIfeatureCallStatus"));
		 * Reporting.logReporter(Status.INFO, "API SIZE : " + apiOperation.size());
		 * Reporting.logReporter(Status.INFO, "API Operation Request: " +
		 * apiOperation.get("spidAPIfeatureCallRequest"));
		 * 
		 * Reporting.logReporter(Status.INFO, "API Operation response: " +
		 * apiOperation.get("spidAPIfeatureCallResponse"));
		 ...
		 * 
		 * String expectedSPID = lsmsTestData.getString("EXPECTED_SPID"); String resp =
		 * XmlUtils.toXml(apiOperation.get("spidAPIfeatureCallResponse")); String
		 * actualSPID = GenericUtils.returnKeyValueFromXMLNode(resp, "lspId");
		 * Validate.assertEquals(actualSPID, expectedSPID, "SPID is not as expected",
		 * true); Reporting.printAndClearLogGroupStatements();
		 * 
		 ...
		 * SmartDesktopDashboardPageSteps.submitTNForChangeFromSubscriberTabForWLN_SD(
		 * sdTestData, sub, ban, foreignSub, foreignBan);
		 * WebDriverSteps.closeTheBrowser();
		 * Reporting.printAndClearLogGroupStatements();
		 * 
		 *//**
		...
		*//*** STEPS FOR CANCEL AND CLONE RQUEST FROM PAC ***/
		 ...
		 * "preValidation"); PACDashboardPageSteps.cancelRequestFromPAC(pacTestData);
		 * 
		 * PACDashboardPage PACDashboardPage = new PACDashboardPage();
		 * BaseSteps.Waits.waitForElementToBeClickableLongWait(PACDashboardPage.
		 * refreshBtn); BaseSteps.Clicks.clickElement(PACDashboardPage.refreshBtn);
		 * 
		 * PACDashboardPageSteps.cloneRequestWithANHIndicatorAsYFromPAC(pacTestData,
		 * foreignBan);
		 * 
		 ...
		 * Validate.assertEquals(actualPortStatus, expectedPortStatus,
		 * "Port status is not as expected", false);
		 * 
		 ...
		 * SMGICPPortSelectionPageSteps.navigateToSMGICPPortSelectionPage(); String
		 * portType = smgTestData.getString("PORT_TYPE_SELECT_PORT_IN"); String
		 * newNetwork = smgTestData.getString("TELUS_TU04_MOBILITY");
		 * SMGICPPortSelectionPageSteps.queryFromICPPortSelectionPagePortIn(foreignSub,
		 * portType, newNetwork);
		 * 
		 * SMGICPPortSelectionPage SMGICPPortSelectionPage = new
		 * SMGICPPortSelectionPage(); String expectedICPPortStatus =
		 * smgTestData.getString("WAITING_FOR_RESPONSE_STATUS");
		 * BaseSteps.Waits.waitGeneric(500);
		 * BaseSteps.Debugs.scrollToElement(SMGICPPortSelectionPage.
		 * smgICPPortStatusFromICPPortSelection); String actualICPPortStatus =
		 * SMGICPPortSelectionPage.getICPPortStatusFromICPPortSelection();
		 ...
		 * RCMPageSteps.verifySubscriberStatusFromRCM(rcmTestData, foreignSub,
		 * "active"); RCMPageSteps.verifyPortingStatusFromRCM(rcmTestData, "pis");
		 * WebDriverSteps.closeTheBrowser();
		 * Reporting.printAndClearLogGroupStatements();
		 * 
		 ...
		 * LoginPageSteps.appLogin_PAC(); PACSearchPageSteps.searchPhoneNo(foreignSub);
		 * PACDashboardPageSteps.handleMultipleRecordsDisplayed();
		 ...
		 * SMGICPPortSelectionPageSteps.navigateToSMGICPPortSelectionPage(); String
		 * portType1 = smgTestData.getString("PORT_TYPE_SELECT_PORT_IN"); String
		 * newNetwork1 = smgTestData.getString("TELUS_TU04_MOBILITY");
		 * SMGICPPortSelectionPageSteps.queryFromICPPortSelectionPagePortIn(foreignSub,
		 * portType1, newNetwork1);
		 * 
		 * SMGICPPortSelectionPage SMGICPPortSelectionPage1 = new
		 * SMGICPPortSelectionPage(); String expectedICPPortStatus1 =
		 * smgTestData.getString("CANCELED_STATUS"); BaseSteps.Waits.waitGeneric(500);
		 * BaseSteps.Waits
		 * .waitForElementToBeClickableLongWait(SMGICPPortSelectionPage1.
		 * smgICPPortStatusFromICPPortSelection);
		 * BaseSteps.Debugs.scrollToElement(SMGICPPortSelectionPage1.
		 * smgICPPortStatusFromICPPortSelection); String actualICPPortStatus1 =
		 * SMGICPPortSelectionPage1.getICPPortStatusFromICPPortSelection();
		 ...
		 * CODASearchPageSteps.verifySubscriberStatusFromCODA(codaTestData, foreignSub,
		 * "active"); WebDriverSteps.closeTheBrowser();
		 * Reporting.printAndClearLogGroupStatements();
		 * 
		 * 
		 *//*** STEPS FOR PCP STATUS FROM RCM ***//*
													 * Reporting.setNewGroupName("RCM - PCP STATUS VALIDATION");
													 * RCMPageSteps.verifySubscriberStatusFromRCM(rcmTestData,
													 * foreignSub, "active");
