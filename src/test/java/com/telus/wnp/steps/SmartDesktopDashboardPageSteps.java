...
public class SmartDesktopDashboardPageSteps extends BaseSteps {
	 ...
	 * 
	 * @param testDataJson This is the input test data json object.
	 * @param sub          This is the subscriber TN.
	 * @param ban          This is the subscriber ban.
	 * @return nothing.
	 */
	public static void submitTNForChangeFromSubscriberTab_SD(JSONObject testDataJson, String sub, String ban,
		...
		try {
			...
			/*
			 * Validate.assertTrue(portedSubDetails.contains(GenericUtils.
			 * getSystemDateInMMDDYYYY()), "Change Request Validation Failed", true,
			 * "Change Request Validation for Requested Date Passed");
			 */
	 ...
	 * communication history tab and verify SMS entry
	 */
	public static void navigateToCommunicationHistoryTabToVerifySMSDetails_NEWLogin(JSONObject testDataJson,
			String sub) {
		SmartDesktopDashboardPage SmartDesktopDashboardPage = new SmartDesktopDashboardPage();
	...
	public static void openPreviewAndCaptureSMSDetails(String sub) {
		...
		try {
			...
			Reporting.logReporter(Status.INFO,
					"Captured SMS Details for sub : " + sub + " is : " + actualMessageContains);
	...
	/**
	 * Method Description: To Capture Sub System Failure logs from Smart Desktop
	 */
	public static void logSubSystemFailureDetails() {

		String errorLog = "";
		SmartDesktopDashboardPage smartDesktopDashboardPage = new SmartDesktopDashboardPage();
		try {
			BaseSteps.Waits.waitForElementVisibilityLongWait(smartDesktopDashboardPage.showErrLogDetails);
			Validate.takeStepScreenShot("SD - SUB-SYSTEM FAILURE OCCURRED", Status.INFO);
			BaseSteps.Debugs.scrollToElement(smartDesktopDashboardPage.showErrLogDetails);
			BaseSteps.Clicks.clickElement(smartDesktopDashboardPage.showErrLogDetails);

			BaseSteps.Waits.waitForElementVisibilityLongWait(smartDesktopDashboardPage.stackTraceLogDetails);
			BaseSteps.Debugs.scrollToElement(smartDesktopDashboardPage.stackTraceLogDetails);
			errorLog = smartDesktopDashboardPage.getStackTrace();
			Reporting.logReporter(Status.INFO, "STACK TRACE FOR SUB SYSTEM FAILURE: - " + errorLog);
		} catch (Exception e) {

			Reporting.logReporter(Status.DEBUG, "SUB SYSTEM FAILURE NOT DISPLAYED");
		}
